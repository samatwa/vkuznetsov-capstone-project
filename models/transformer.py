...
class TransformerModel(nn.Module):
    def __init__(
        ...
        # batch_first=True: input/output shape is [batch, seq_len, d_model]
        # This enables PyTorch's Fast Path (FlashAttention kernels on GPU)
        # and eliminates scaled_dot_product_attention warnings.
        encoder_layers = nn.TransformerEncoderLayer(
            ...
            batch_first=True,
            norm_first=True,  # Pre-LN: more stable training for non-ReLU activations
        )
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, nlayers)

        self.encoder = nn.Embedding(ntoken, d_model, padding_idx=0)
        # Learnable positional encoding — simpler and often matches sinusoidal quality
    ...
    def forward(self, src, src_key_padding_mask=None):
        # src: [batch, seq_len]  (batch_first=True in data.py collate_fn)
        seq_len = src.size(1)
        ...
        # Build padding mask from <pad>=0 tokens so attention ignores them
